...
public class PrescriptionController {
  ...
  public ApiResponse<PrescriptionDtos.PrescriptionView> create(
          @RequestHeader("X-User-Id") String doctorId,
          @Parameter(name = "X-Idempotency-Key", in = ParameterIn.HEADER, required = true, description = "Client-generated key; duplicate POSTs with the same key return the original prescription.")
          @RequestHeader("X-Idempotency-Key") String idempotencyKey,
          @Valid @RequestBody PrescriptionDtos.CreatePrescriptionRequest r) {
    return ApiResponse.ok(service.create(doctorId, idempotencyKey, r), "Created");
  }
